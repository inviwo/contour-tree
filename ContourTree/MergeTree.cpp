...
void MergeTree::output(TreeType tree) {
    if (tree == TypeContourTree) {
        ctree.output();
        nodeIDs = ctree.nodeIDs;
        nodeFnVals = ctree.nodeFnVals;
        nodeTypes = ctree.nodeTypes;
        arcList = ctree.arcList;
        vToArcMap = ctree.vToArcMap;
        noNodes = ctree.nodeIDs.size();
        noArcs = ctree.arcNo;
        return;
    }
